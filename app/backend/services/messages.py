...
class MessagesService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Messages]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Messages]:
        """Get messages by ID"""
        try:
    ...
    async def get_list(
        ...
        """Get paginated list of messagess"""
        try:
            ...
            stmt = select(Messages, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Messages, field_name):
                        stmt = stmt.order_by(getattr(Messages, field_name).desc())
                else:
                    if hasattr(Messages, sort):
                        stmt = stmt.order_by(getattr(Messages, sort))
            else:
                stmt = stmt.order_by(Messages.id.desc())
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Messages]:
        """Update messages"""
        try:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Messages]:
        ...
        try:
            ...
            column = getattr(Messages, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Messages).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
