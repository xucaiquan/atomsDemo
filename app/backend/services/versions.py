...
class VersionsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Versions]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Versions]:
        """Get versions by ID"""
        try:
    ...
    async def get_list(
        ...
        """Get paginated list of versionss"""
        try:
            ...
            stmt = select(Versions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Versions, field_name):
                        stmt = stmt.order_by(getattr(Versions, field_name).desc())
                else:
                    if hasattr(Versions, sort):
                        stmt = stmt.order_by(getattr(Versions, sort))
            else:
                stmt = stmt.order_by(Versions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Versions)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Versions]:
        """Update versions"""
        try:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Versions]:
        ...
        try:
            ...
            column = getattr(Versions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Versions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
