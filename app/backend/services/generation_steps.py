...
# ------------------ Service Layer ------------------
class Generation_stepsService:
    """Service layer for Generation_steps operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Generation_steps]:
        """Create a new generation_steps"""
        try:
            obj = Generation_steps(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created generation_steps with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating generation_steps: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Generation_steps]:
        """Get generation_steps by ID"""
        try:
            query = select(Generation_steps).where(Generation_steps.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching generation_steps {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of generation_stepss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Generation_steps, field):
                        column = getattr(Generation_steps, field)
            ...
            stmt = select(Generation_steps, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Generation_steps, field_name):
                        stmt = stmt.order_by(getattr(Generation_steps, field_name).desc())
                else:
                    if hasattr(Generation_steps, sort):
                        stmt = stmt.order_by(getattr(Generation_steps, sort))
            else:
                stmt = stmt.order_by(Generation_steps.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Generation_steps)
                for condition in conditions:
            ...
            logger.error(f"Error fetching generation_steps list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Generation_steps]:
        """Update generation_steps"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Generation_steps {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated generation_steps {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating generation_steps {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete generation_steps"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Generation_steps {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted generation_steps {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting generation_steps {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Generation_steps]:
        """Get generation_steps by any field"""
        try:
            if not hasattr(Generation_steps, field_name):
                raise ValueError(f"Field {field_name} does not exist on Generation_steps")
            column = getattr(Generation_steps, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Generation_steps).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching generation_steps by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Generation_steps]:
        """Get list of generation_stepss filtered by field"""
        try:
            if not hasattr(Generation_steps, field_name):
                raise ValueError(f"Field {field_name} does not exist on Generation_steps")
            column = getattr(Generation_steps, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Generation_steps.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching generation_stepss by {field_name}: {str(e)}")
            raise