...
async def initialize_mock_data():
    ...
    This remains an explicit generation/deployment utility and is not injected
    into runtime app startup.
    """
    # Legacy flag compatibility: only a true value skips this explicit operation.
    if get_env_bool("MGX_IGNORE_INIT_DATA", default=False):
        logger.info("Mock-data initialization skipped by legacy MGX_IGNORE_INIT_DATA")
        return
    ...
    async def load_file(data_file: Path):
        async with semaphore:
            try:
                await _load_table_from_file(data_file, engine)
...
async def _load_table_from_file(data_file: Path, engine: AsyncEngine):
