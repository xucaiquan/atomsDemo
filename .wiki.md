...
Atoms Studio 是面向应用原型和业务页面生成的智能开发平台。用户通过自然语言描述需求，系统自动分析需求、设计页面并生成完整 HTML，在隔离沙箱中预览可交互应用。项目、版本、生成步骤和对话记录会持久化保存，支持历史版本切换、非破坏性回滚、持续迭代、源代码查看和恢复。

系统采用异步多阶段生成流程，支持历史上下文、指代消解、增量生成、代码续写、整篇重跑、HTML 校验、控件保留校验、定向修复、版本切换、取消、失败展示、预算控制和任务恢复。每轮生成基于最近成功版本创建独立新版本，历史版本内容和哈希保持不变。

版本切换现已支持在 v1、v2、v3 等历史版本之间任意切换。切换按版本序号获取对应 HTML、摘要和 SHA-256，源码查看与预览共享所选版本内容，并保留用户当前所在视图，不再强制跳回预览页。回滚仍采用创建新版本的非破坏性语义。
...
- **产物级控件保留校验**
  ...
  - 检查控件文案是否仍存在于新版本完整 HTML 中。
  - 发现缺失时触发一次定向修复，仅在缺失项减少时采纳修复结果。
  - 修复失败或预算不足时保留原始产物，并记录 `summary.missing_controls`。
...
- **增量生成与版本隔离**
  - 以成功版本 HTML 为基线创建独立新版本。
  - 原版本内容、摘要和哈希不会被新版本覆盖。
  - 阶段三提示词要求保留原有功能、按钮文案和示例数据。
  - 当前不实现 BSDiff、AST 补丁或局部 DOM 注入，模型仍输出完整 HTML。

- **版本切换与回滚**
  - 通过版本序号读取对应版本的 HTML、摘要和 SHA-256。
  - 多版本之间可任意切换，源码和预览同步展示所选版本。
  - 切换时保留当前预览或源码视图，不强制改变用户视图。
  - 回滚通过创建新版本实现，历史版本始终保留。
  - 工具栏展示当前 `v{n}`、内容大小和 SHA 标识，便于核对版本身份。
...
- **自动化测试与验收**
  ...
  - 新增多版本切换一致性测试，验证 v1/v2/v3 内容、摘要和哈希按序号正确返回。
  - 验证版本历史不可变，防止切换或生成过程覆盖旧版本。
  - 后端 163 项测试通过，前端 lint 和 build 通过。
...
| 工作区页面 | `frontend/src/pages/Index.tsx` | 管理生成轮询、版本切换、恢复、停止、错误和修复状态展示 |
| 源码查看器 | `frontend/src/components/CodeViewer.tsx` | 展示所选版本源码和哈希，提供兼容复制及状态反馈 |
| 版本切换 | `frontend/src/components/VersionSwitcher.tsx` | 切换版本、请求版本内容并展示版本标识 |
| 源码预览一致性 | `backend/tests/test_source_preview_consistency.py` | 验证多版本切换时源码与预览使用正确版本内容 |
...
- **版本一致性**：按序号版本读取、非破坏性回滚、SHA-256 内容校验、源码预览一致性
